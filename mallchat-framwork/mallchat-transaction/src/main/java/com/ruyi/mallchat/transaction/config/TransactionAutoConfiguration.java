package com.ruyi.mallchat.transaction.config;

import com.ruyi.mallchat.transaction.aspect.SecureInvokeAspect;
import com.ruyi.mallchat.transaction.config.interfaces.SecureInvokeConfigurer;
import com.ruyi.mallchat.transaction.dao.SecureInvokeRecordDao;
    ...
    void setConfigurers(ObjectProvider<SecureInvokeConfigurer> configurers) {
        Supplier<SecureInvokeConfigurer> configurer = SingletonSupplier.of(() -> {
            ...
            if (candidates.size() > 1) {
                throw new IllegalStateException("只能存在一个SecureInvokeConfigurer");
            }
