package com.ruyi.mallchat.common.websocket.service.impl;
...
import com.ruyi.mallchat.common.common.config.ThreadPoolConfig;
import com.ruyi.mallchat.common.common.constant.RedisKey;
import com.ruyi.mallchat.common.common.event.UserOfflineEvent;
...
import com.ruyi.mallchat.common.user.domain.dto.WSChannelExtraDTO;
...
public class WebSocketServiceImpl implements WebSocketService {
    ...
    public void handleLoginReq(Channel channel) {
        //生成随机不重复的登录事件码,并将channel存在本地cache中
        Integer code = generateLoginCode(channel);
        //请求微信接口，获取登录码地址
        WxMpQrCodeTicket wxMpQrCodeTicket = wxMpService.getQrcodeService().qrCodeCreateTmpTicket(code, Math.toIntExact(EXPIRE_TIME.getSeconds()));
        //返回给前端（channel必在本地）
        sendMsg(channel, WSAdapter.buildLoginResp(wxMpQrCodeTicket));
    }

    /**
     * 生成不重复的登录的code，微信要求最大不超过int的存储极限
    ...
    private Integer generateLoginCode(Channel channel) {
        int loginCode;
        do {
            //redis存储自增loginCode并返回值，本地cache时间必须比redis key过期时间短，否则会出现并发问题
            loginCode = RedisUtils.integerInc(RedisKey.getKey(LOGIN_CODE), Math.toIntExact(EXPIRE_TIME.toMinutes()), TimeUnit.MINUTES);
        } while (WAIT_LOGIN_MAP.asMap().containsKey(loginCode));
        //储存loginCode与channel的关系在本地
    ...
    /**
     * (channel必在本地)登录成功，更新状态、通知前端、发布上线事件
     */
    ...
    private void online(Channel channel, Long uid) {
        ...
        //在本地保存 在线用户的uid 和 websocket的channel 的对应关系
    ...
    private boolean offline(Channel channel, Optional<Long> uidOptional) {
        //移除本地保存的 已连接websocket 和 用户信息对应 关系
        ONLINE_WS_MAP.remove(channel);
        if (uidOptional.isPresent()) {//移除本地保存的 在线用户的uid 和 websocket 的对应关系
    ...
    public void sendToUid(WSBaseResp<?> wsBaseResp, Long uid) {
        //从 本地保存的 在线用户的uid 和 websocket 的对应关系中获取channel
